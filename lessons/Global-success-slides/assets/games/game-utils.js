...
class UIComponents {
    ...
    let controlsHTML = `
    <div class="w-full my-2 flex flex-col sm:flex-row sm:justify-center gap-2 sm:gap-4 items-stretch transition-transform duration-200">
    `;

    // Stats group (row 1 on mobile, left side on desktop)
    controlsHTML += `
    <div class="w-full sm:w-auto flex flex-row gap-1 sm:gap-4 justify-stretch sm:justify-start">
    `;
    if (showTimer) {
    controlsHTML += `
        <div id="timer-${gameId}" 
        class="bg-yellow-100 text-yellow-700 font-bold px-2 py-1 rounded-full shadow-lg text-center text-xs sm:text-sm w-full sm:w-28"
        style="display: none;">
    ...
    controlsHTML += `
        <div id="score-${gameId}" 
        class="bg-purple-100 text-purple-700 font-bold px-2 py-1 rounded-full shadow-lg text-center text-xs sm:text-sm w-full sm:w-28"
        style="display: none;">
    ...
    if (showMoves) {
    controlsHTML += `
        <div id="moves-${gameId}__container"
        class="bg-green-100 text-green-700 font-bold px-2 py-1 rounded-full shadow-lg text-center text-xs sm:text-sm w-full sm:w-32"
        style="display: none;">
        🔄 Moves: <span id="moves-${gameId}">0</span>
        </div>
    `;
    }
    controlsHTML += `</div>`;
    ...
    controlsHTML += `
    <div class="w-full sm:w-auto flex flex-row gap-1 sm:gap-4 justify-stretch sm:justify-end">
    `;
    if (showStart) {
    controlsHTML += `
        <button id="start-btn-${gameId}" 
        class="bg-green-500 hover:bg-green-600 text-white font-semibold px-3 py-1 rounded-full shadow-lg transition-all duration-200 text-xs sm:text-sm w-full sm:w-auto">
        ${startText}
        </button>
    `;
    }
    ...
    controlsHTML += `
        ...
        class="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold px-3 py-1 rounded-full shadow-lg transition-all duration-200 text-xs sm:text-sm w-full sm:w-auto"
        style="display: none;">
        ${restartText}
        </button>
    `;
