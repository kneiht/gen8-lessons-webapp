...
class MultipleChoiceEnViGame extends GameBase {
    constructor() {
        super('Multiple Choice Game - English to Vietnamese', 10);
    ...
    getGameInfo() {
        return {
            name: 'Multiple Choice - English to Vietnamese',
    ...
    createGameQuestions(words) {
        ...
        words.forEach(word => {
            ...
            const allOptions = [word.vietnamese_meaning, ...wrongAnswers];
            const shuffledOptions = GameUtils.shuffleArray(allOptions);
            
            questions.push({
                english: word.english_word,
                correct: word.vietnamese_meaning,
    ...
    getRandomWrongAnswers(correctWord, allWords, count) {
        const wrongOptions = allWords
            .filter(w => w.vietnamese_meaning !== correctWord.vietnamese_meaning)
            .map(w => w.vietnamese_meaning);
        
        return GameUtils.selectRandomWords(
            wrongOptions.map(meaning => ({ vietnamese_meaning: meaning })), 
            count
        ).map(w => w.vietnamese_meaning);
    }
            ...
            <div class="question-card bg-white rounded-xl shadow-lg p-3 ">
                ...
                <div class="flex flex-col lg:flex-row gap-3 items-center justify-center">
                    <div class="text-center mb-0 lg:mb-0 lg:mr-8 flex-shrink-0">
                        <h3 class="text-3xl font-bold text-blue-600 mb-2">${question.english}</h3>
                        ${question.pronunciation ? `<p class="text-sm text-gray-500">${question.pronunciation}</p>` : ''}
                    </div>
                    <!-- Options -->
